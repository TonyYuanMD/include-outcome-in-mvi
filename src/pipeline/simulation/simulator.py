...
class SimulationStudy:
    def __init__(self, n=1000, p=5, num_runs=2, continuous_pct=0.4, sparsity=0.3, 
                 include_interactions=False, include_nonlinear=False, include_splines=False, seed=123):
        self.n = n
        self.p = p
        self.num_runs = num_runs
        self.continuous_pct = continuous_pct
        self.integer_pct = 1 - continuous_pct - sparsity
        if self.integer_pct < 0:
            raise ValueError("integer_pct negative. Adjust params.")
        self.sparsity = sparsity
        self.include_interactions = include_interactions
        self.include_nonlinear = include_nonlinear
        self.include_splines = include_splines
        self.seed = seed
        self.output_dir = f'results/report/n_{n}_p_{p}_runs_{num_runs}_cont_{continuous_pct}_sparse_{sparsity}/'
