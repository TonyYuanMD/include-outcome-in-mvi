...
def generate_data(n=1000, p=5, continuous_pct=0.4, integer_pct=0.4, sparsity=0.3,
    ...
    nonzero_indices = np.random.choice(num_predictors, num_nonzero, replace=False)
    beta = np.zeros(num_predictors + 1)
    beta[0] = np.random.normal(0, 1)
    beta[nonzero_indices + 1] = np.random.normal(0, 1, num_nonzero)
    
    # Generate outcomes
    design_matrix = np.column_stack([np.ones(n)] + [data[cov] for cov in covariates])
    logits = design_matrix @ beta
    probs_y = 1 / (1 + np.exp(-logits))
    data['y'] = np.random.binomial(1, probs_y)
    data['y_score'] = design_matrix @ beta + np.random.normal(0, 1, n)
